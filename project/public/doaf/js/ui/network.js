...
export function mountNetwork(canvas, entries, onPick) {
  ...
  function draw(t) {
    ...
    ctx.strokeStyle = 'rgba(255,255,255,.05)'; [0.25, 0.5, 0.75].forEach((k) => { ctx.beginPath(); ctx.ellipse(W / 2, H / 2, W * 0.5 * k * 1.7, H * 0.5 * k * 1.7, 0, 0, 7); ctx.stroke(); });
    for (let i = 1; i < nodes.length; i++) { const n = nodes[i], on = i === hover; ctx.strokeStyle = on ? 'rgba(98,207,255,.9)' : 'rgba(255,43,43,.32)'; ctx.beginPath(); ctx.moveTo(nodes[0].x, nodes[0].y); ctx.lineTo(n.x, n.y); ctx.stroke();
      const nx = nodes[(i % (nodes.length - 1)) + 1]; if (nx !== n) { ctx.strokeStyle = 'rgba(98,207,255,.10)'; ctx.beginPath(); ctx.moveTo(n.x, n.y); ctx.lineTo(nx.x, nx.y); ctx.stroke(); } }
    nodes.forEach((n, i) => { const on = i === hover, pulse = n.hub ? 1 + Math.sin(t * 0.003) * 0.15 : 1; ctx.fillStyle = n.hub ? '#ff2b2b' : on ? '#62cfff' : '#d6dbe2'; ctx.shadowColor = n.hub ? '#ff2b2b' : '#62cfff'; ctx.shadowBlur = n.hub || on ? 16 : 0; ctx.beginPath(); ctx.arc(n.x, n.y, n.r * pulse + (on ? 2 : 0), 0, 7); ctx.fill(); ctx.shadowBlur = 0;
      if (n.hub || on) { ctx.fillStyle = '#fff'; ctx.font = '600 11px IBM Plex Mono, monospace'; ctx.textAlign = 'center'; ctx.fillText(n.label, n.x, n.y - 16); } });
